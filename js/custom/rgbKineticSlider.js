(function () {
  ...
  window.rgbKineticSlider = function (options) {
    ...
    const autoTransition = () => {
      let tempIndex;
      let slideIndex = 1;      
      let allSlides = $(".slider-nav").children();

      $('.first').addClass('active')

      return setInterval(() => {
        tempIndex = slideIndex % options.slideImages.length
        
        slideTransition(tempIndex)
    ...
    if (options.nav == true) {
      ...
      for (let i = 0; i < nav.length; i++) {
        ...
        navItem.onclick = function (event) {
          ...
          //don't run the transition again if the element is already active
          if($(this).hasClass('active')){
            return false;
          }

          //remove the active class from all other elements and add it to the current one
          ...
          let slideNumber;
          switch(this.getAttribute('data-nav') ){
            case 'first':
              slideNumber = 0;
              break;
            case 'second':
